...
const DEFAULT_CONFIG = {
  ...
  // If true, "not reporting" also treats Homey's own lastSeenAt (confirmed
  // connectivity/mesh keep-alive) as a sign of life, not just capability value
  // changes - whichever is newer counts. Fixes false positives for event-driven
  // devices (e.g. Zigbee routers/contact sensors) that rarely change a capability
  // value but are still clearly connected. Off by default: capability-only detection
  // also catches a device that's connected but whose data itself is stuck, which this
  // would miss.
