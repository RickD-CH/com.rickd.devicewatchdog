 ...
 * capabilities are momentary/stateless - a virtual "button", a scene-trigger helper, etc. -
 * never carries a `lastUpdated` on any capability, because Homey doesn't record one for a
 * `button` press (it only fires a Flow trigger). Running the "not reporting" check on such a
 * device would flag it forever with nothing the user could do about it (confirmed on the
 * community thread for virtual "button" devices from com.arjankranenburg.virtual).
 *
...
function canCheckStaleness(device) {
  ...
  return caps.some((cap) => String(cap).split('.')[0] !== 'button');
}
