 ...
 * staleness check only when a device's rule explicitly opts in (includeBatteryForReporting)
 * - see computeDeviceStatus. Off by default: many devices report their battery level on its
 * own cadence, independent of whether the device's actual measured value changed, so without
 * this a frozen primary sensor value (e.g. measure_temperature stuck for weeks) hides behind
 * a battery capability that's still ticking every few hours - the device keeps showing OK.
 * Kept as its own set (not reused from NON_INFORMATIVE_STALENESS_CAP_BASES above) since the
 * two lists serve different purposes: that one gates whether staleness is checked AT ALL,
 * this one only filters which capability's timestamp counts once it is.
 */
...
function computeDeviceStatus(device, rule, config) {
  ...
  if (!custom.onlyCheckBattery && canCheckStaleness(device)) {
    const includeBattery = !!custom.includeBatteryForReporting;
