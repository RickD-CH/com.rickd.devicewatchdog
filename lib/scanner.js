 ...
 * @returns {{count:number, avgIntervalMs:?number, maxIntervalMs:?number, recommendedHours:?number}}
 */
function computeUpdateStats(timestamps) {
  ...
  if (count < MIN_UPDATE_STATS_SAMPLES) {
    return {
      count, avgIntervalMs: null, maxIntervalMs: null, recommendedHours: null,
    };
  }
  ...
  return {
    count, avgIntervalMs, maxIntervalMs, recommendedHours,
  };
}
