...
/**
 * Turns a device's rolling window of recent "it reported something" timestamps into a
 * summary (average/max interval) and a suggested "not reporting" threshold. Pure - the
 * caller (app.js) owns collecting/persisting the timestamps; this only does the math, so it
 * can be unit tested without any HomeyAPI/settings involved.
 * @param {number[]} timestamps - recent update times in ms, ascending (oldest first)
 * @returns {{count:number, timestamps:number[], avgIntervalMs:?number, maxIntervalMs:?number, recommendedHours:?number}}
 */
function computeUpdateStats(timestamps) {
  const list = Array.isArray(timestamps) ? timestamps : [];
  const count = list.length;
  if (count < MIN_UPDATE_STATS_SAMPLES) {
    return {
      count, timestamps: list, avgIntervalMs: null, maxIntervalMs: null, recommendedHours: null,
    };
  }
  ...
  return {
    count, timestamps: list, avgIntervalMs, maxIntervalMs, recommendedHours,
  };
}
