...
function isExcludedByGlobalFilters(device, zoneMap, config) {
  ...
  // Empty/unset includedDeviceClassPattern means "no class restriction" - every device
  // class is allowed. Only exclude by class when a pattern is actually configured
  // (there's no Settings UI for this anymore; it only applies if set directly via settings).
  const classPattern = safeRegExp(config.includedDeviceClassPattern);
  if (classPattern && (!device.class || !classPattern.test(device.class))) return true;
